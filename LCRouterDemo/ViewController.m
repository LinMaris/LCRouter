...
-(void)btnClick:(UIButton *)sender
{
    switch (sender.tag) {
        ...
        {
            // 从plist文件中匹配
//            [[LCRouter sharedLCRouter] pushURLString:@"maris://twoitem" params:@{@"array" : @[@"a",@"b",@"c"]} animated:true];
            // 直接匹配
...
-(void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [LCRouter openURLString:@"lili" withParams:@{@"name" : @"helloWorld"} completion:^(id result) {
        NSLog(@"result:==>%@",result);
    }];
...
{
    [LCRouter registerURLString:@"lili" handler:^(NSDictionary *params) {
        NSLog(@"params:%@",params);
