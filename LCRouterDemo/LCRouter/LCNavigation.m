...
- (NSDictionary *)paramsURL:(NSURL *)url {
    ...
    NSDictionary* pairs = [NSDictionary dictionary];
    if (NSNotFound != [url.absoluteString rangeOfString:@"?"].location) {
        ...
        pairs = [self paramsFromString:paramString];
    }
    
    if (url.scheme) {
        pairs = [self paramsFromString:url.host];
    }else if(url.path){
        pairs = [self paramsFromString:url.path];
    }
    
    return [NSDictionary dictionaryWithDictionary:pairs];
}

-(NSDictionary *)paramsFromString:(NSString *)paramString
{
